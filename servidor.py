    ...
    try:
        ...
        if type == b'S':
            ...
            processar_nonce(num_transacao, nonce, client_name)
                ...
                # IMPLEMENTAR O PROTOCOLO V (Transação inválida)

                # IMPLEMENTAR O PROTOCOLO R (Transação válida)

                    # IMPLEMENTAR O PROTOCOLO I (Broadcast avisando todos que a transação foi validada)
...
def gerar_transacoes():
    while True:
        try:
            transacao = input("Informe uma nova transação: ")
